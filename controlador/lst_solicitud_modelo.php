...
class lst_solicitud_model{
    ...
    */
	...
	public function select_solicitud_compra($searchQuery,$columnName,$columnSortOrder, $row,$rowperpage){
		...
		$sql="SELECT s.* , DATE_FORMAT(s.fecha,'%d/%m/%Y') AS fechaf, to_days(s.fecha) as dias,
					DATE_FORMAT(s.fechatc,'%d/%m/%Y') AS fechatcf,
					IFNULL(kgxempaque*cantidad,0) AS total,
		...
		$sql="SELECT DISTINCT  d.*,s.empaque, s.kgxempaque, s.codtipoempaque,
			...
			FROM lst_listaintegrada_det d left JOIN 
				...
				LEFT JOIN (
					...
					WHERE se.flag='1' AND de.flag='1' AND se.estado in ('a','e') and se.codsolicitud!=$codsolicitud
				...
				LEFT JOIN (
					...
					WHERE so.flag='1' AND co.flag='1' AND so.estado in ('a','e') and so.codsolicitud!=$codsolicitud
	...
	public function select_resumen_listaint($codsolicitud){
		unset($this->listas);
