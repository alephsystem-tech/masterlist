...
class kpi_reporte_model{
    ...
    */
	...
	public function selec_certificador_reporte($id_pais,$tipokpi=null){
		unset($this->listas);
		$this->listas=[];
		$sql="select distinct certificador as pais
				from kpi_importar 
	...
	public function select_kpigraf_pais($codprograma,$fechai,$fechaf,$auditor,$pais,$oficinacontrato,$anio,$id_pais){
		unset($this->listas);
		$this->listas=[];
		$sql="SELECT oficina, COUNT(item) AS total, round(AVG(stiempo),2) as stiempo,
			...
			FROM kpi_importar INNER JOIN kpi_importar_cab ON kpi_importar.id =kpi_importar_cab.id 
				AND	kpi_importar.codprograma =kpi_importar_cab.codprograma
			WHERE flgcerrado='1' AND kpi_importar_cab.flag='1' AND fecha!='0000-00-00' ";
	...
	public function select_kpigraf_pais_cer($codprograma,$fechai,$fechaf,$auditor,$pais,$oficinacontrato,$anio,$id_pais){
		...
		$sql="SELECT certificador as oficina, COUNT(item) AS total, 
			...
			FROM kpi_importar INNER JOIN kpi_importar_cab ON kpi_importar.id =kpi_importar_cab.id 
				AND	kpi_importar.codprograma =kpi_importar_cab.codprograma
			WHERE flgcerrado='1' AND kpi_importar_cab.flag='1' AND fechaevaluacion!='0000-00-00' and kpi_importar.tipokpi='cer' ";
	...
	public function select_kpigraf_pais_mes($codprograma,$pais,$auditor,$anio,$oficinacontrato,$id_pais){
		unset($this->listas);
		$this->listas=[];
	...
	public function select_kpigraf_oneAudit($tipokpi,$auditor,$codprograma,$fechai,$fechaf,$oficina,$anio,$pais,$oficinacontrato){
		unset($this->listas);
		$this->listas=[];
		$sql="SELECT fullauditor,certificadorname,pais, stiempo,scalidad,svolumen,sgestion,sproc,project_id,sfinal,project,
