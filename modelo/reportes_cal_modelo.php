	...
	*****************************************/
	...
	public function select_reporte_planificacion($searchQuery,$columnName,$columnSortOrder, $row,$rowperpage,$fechai,$fechaf){
		...
		$sql="SELECT DISTINCT
			...
			FROM    prg_calendario calendario LEFT JOIN 
				prg_proyecto proyecto ON trim(calendario.id_proyecto) = proyecto.project_id AND proyecto.project_id <> '' and proyecto.id_pais=calendario.id_pais LEFT JOIN 
				prg_tipoactividad tipoactividad ON calendario.id_tipoactividad = tipoactividad.id_tipoactividad JOIN 
				prg_auditor auditor ON calendario.id_auditor = auditor.id_auditor AND auditor.flag='1' LEFT JOIN 
	...
	*****************************************/
	...
	public function select_resumen_viaticos_auditor_monto($id_pais,$anio,$G_tc){
		...
		$sql="SELECT
			...
			SUM(CASE flag_rendicion WHEN 2 THEN ifnull(monto_dolares,0) + ifnull(monto_soles/$G_tc,0) ELSE 0 END) AS entdolares,
			SUM(CASE flag_rendicion WHEN 2 THEN ifnull(utizado_monto_dolares,0) + ifnull(utizado_monto_soles/$G_tc,0) ELSE 0 END) 
			- SUM(vista.dolar) AS dolares,
		...
		FROM prg_calendario
			LEFT JOIN (
				SELECT 	sum(IFNULL(CASE tipo_moneda WHEN 'USD' THEN monto ELSE 0 END,0)) + sum(IFNULL(CASE tipo_moneda WHEN 'S' THEN monto/$G_tc ELSE 0 END,0)) AS dolar,
					id_calendario
				FROM 	prg_detalle_viaticos
				WHERE flag='1' AND tipo='CONT'
				GROUP BY id_calendario
				HAVING  dolar>0		 
			) AS vista ON prg_calendario.id=vista.id_calendario
		WHERE prg_calendario.flag='1' AND id_pais='$id_pais' AND  year(inicio_evento)=$anio AND id_estadoactividad!=3  
		AND ( IFNULL(monto_dolares,0) >0 OR IFNULL(monto_soles,0) >0 ) 
		GROUP BY month(inicio_evento) ";
	...
	public function select_resumen_viaticos_excel($id_pais,$anio,$id_auditor,$mes,$tc){
		...
		$sql="SELECT asunto, DATE_FORMAT(inicio_evento,'%d/%m/%Y %H:%i') AS fechaini, project_id,
				...
				IFNULL(monto_dolares,'0') monto_dolares, IFNULL(monto_soles,'0') monto_soles,
				prg_calendario.usuario_ingreso,
				CASE flag_rendicion 
			...
			FROM prg_calendario INNER JOIN prg_auditor ON prg_calendario.id_auditor = prg_auditor.id_auditor LEFT JOIN
				prg_proyecto ON trim(prg_calendario.id_proyecto)=prg_proyecto.project_id
			WHERE prg_calendario.flag='1' AND prg_calendario.id_pais= '$id_pais'  AND prg_proyecto.id_pais='$id_pais' 
					and ( monto_soles>0 or monto_dolares>0) ";
	...
	public function selec_reporte_programmed_services($searchQuery,$columnName,$columnSortOrder, $row,$rowperpage){
		...
		$sql="SELECT prg_comercial_factura.*,
			...
			FROM prg_calendario
				INNER JOIN prg_proyecto on prg_proyecto.project_id=trim(prg_calendario.id_proyecto)
