...
class inv_producto_model{
    ...
    */
	...
	public function selec_total_producto($searchQuery=null){
		$sql=" SELECT COUNT(distinct inv_producto.codproducto) AS total 
				FROM inv_producto left JOIN 
					...
					LEFT JOIN 
						( SELECT ft.coddetalle, usuariodestino,
							ft.codproducto,codarea ,codsede,
							inv_motivo.motivo, 
							inv_motivo.tipo,
						...
						FROM inv_dettransaccion ft INNER JOIN 
							...
							INNER JOIN (
								SELECT MAX(coddetalle) AS coddetalle, codproducto FROM inv_dettransaccion 
								WHERE flag='1' GROUP BY codproducto) 
							AS temp ON ft.coddetalle=temp.coddetalle
	...
	public function select_producto($searchQuery,$columnName,$columnSortOrder, $row,$rowperpage){
		unset($this->listas); // fechatra fecha_ingreso
		...
		$sql="
			SELECT imei,
				...
				FROM inv_producto LEFT JOIN inv_marca ON  inv_producto.codmarca=inv_marca.codmarca
					...
					INNER JOIN (
								SELECT MAX(coddetalle) AS coddetalle, codproducto , codtransaccion
								FROM inv_dettransaccion 
								WHERE flag='1' 
								GROUP BY codproducto, codtransaccion) 
								AS temp ON d.coddetalle=temp.coddetalle AND d.codtransaccion=temp.codtransaccion
	...
	******************************************/
	...
	public function exportar_reporte_resumido($tipo, $asignado, $codtipo, $clase, $descripcion, $simple) {
	$sql = "SELECT fecha_adquisicion, proveedor, nro_comprobante, moneda, precio_sin_igv, cantidad, precio_con_igv
			FROM inventario
			WHERE tipo = ? AND asignado = ? AND codtipo = ? AND clase = ? AND descripcion LIKE ?";

	$stmt = $this->conn->prepare($sql);
	$stmt->execute([$tipo, $asignado, $codtipo, $clase, "%$descripcion%"]);
